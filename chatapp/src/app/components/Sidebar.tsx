    ...
    <div className="bg-gradient-to-b from-slate-800 to-slate-700 h-full overflow-y-auto p-4 flex flex-col">
      <div className="flex-grow space-y-3">
        ...
        <div className="space-y-2">
          <button
            onClick={addRoom}
            className="w-full glass-button rounded-xl p-3 transition-all duration-200 flex items-center space-x-3 group hover:scale-[1.02]"
          >
            <div className="w-8 h-8 bg-primary-500 group-hover:bg-primary-400 rounded-xl flex items-center justify-center transition-colors">
          ...
          <button
            ...
            className="w-full glass-button rounded-xl p-3 transition-all duration-200 flex items-center space-x-3 group hover:scale-[1.02]"
          >
            <div className="w-8 h-8 bg-red-500 group-hover:bg-red-400 rounded-xl flex items-center justify-center transition-colors">
        ...
        <div className="glass-morphism rounded-xl overflow-hidden">
          <div className="p-3 border-b border-white/10">
            <h3 className="text-white/80 font-medium text-sm">Recent Rooms</h3>
          </div>
            ...
            {rooms.length > 0 ? (
              <ul className="divide-y divide-white/10">
                {rooms.map((room) => (
                  <li
                    key={room.id}
                    className={`flex items-center justify-between p-3 text-white hover:bg-white/10 transition-all duration-200 ${
                      selectedRoom === room.id ? 'bg-primary-500/20 border-l-2 border-primary-400' : ''
                    }`}
                  >
                    <span
                      className="cursor-pointer flex-grow flex items-center space-x-2 hover:scale-[1.02] transition-transform"
                      onClick={() => selectRoom(room.id, room.name)}
                    >
                      <HiChatBubbleLeft className="w-4 h-4 text-white/60 flex-shrink-0" />
                    ...
                    <button
                      ...
                      className="text-red-400 hover:text-red-300 p-1.5 rounded-xl hover:bg-red-500/20 transition-all ml-2 flex-shrink-0 hover:scale-110"
                      title="Delete room"
                    >
                      <FaTrash size={12} />
                    </button>
      ...
      <div className="space-y-2 pt-3 border-t border-white/10">
          ...
          <div className="glass-morphism rounded-xl p-3">
            <div className="flex items-center space-x-2">
              <div className="w-8 h-8 bg-white/10 rounded-xl flex items-center justify-center">
                <HiUser className="w-4 h-4 text-white/80" />
              </div>
        ...
        <button
          ...
          className="w-full glass-button hover:bg-red-600/20 rounded-xl p-3 hover:border-red-500/30 transition-all duration-200 flex items-center space-x-3 group hover:scale-[1.02]"
        >
          <div className="w-8 h-8 bg-white/10 group-hover:bg-red-500 rounded-xl flex items-center justify-center transition-colors">
            <HiArrowRightOnRectangle className="w-4 h-4 text-white/80 group-hover:text-white" />
          </div>
          <span className="text-white/80 group-hover:text-white font-medium">Sign Out</span>
        </button>
      </div>
